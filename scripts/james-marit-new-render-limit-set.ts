/**
 * james-marit-new-render-limit-set.ts
 *
 * Offline density-image render of the james-marit-new limit set in RP³
 ...
 *
 *   node scripts/james-marit-new-render-limit-set.ts [depth] [flags...]
 *
...
if (!existsSync(VIEW_PRESET_PATH)) {
  ...
  log(`    Open the james-marit-new demo in the browser, tune the view, then press`);
...
try {
  ...
  log(`[james-marit-new-render] failed to parse view preset: ${(e as Error).message}`);
  process.exit(1);
}
...
if (
  ...
  log(`[james-marit-new-render] view preset has no generators — was it written by an old demo?`);
  process.exit(1);
}
...
if (typeof VIEW_PRESET.colorScheme === 'string') {
  try {
    ...
    log(`[james-marit-new-render] ignoring preset.colorScheme=${VIEW_PRESET.colorScheme}: ${(e as Error).message}`);
  }
}
if (
...
log(`[james-marit-new-render] loaded james-marit-new-view-preset.json` +
...
function applyFloat(
  ...
  if (predicate(n)) set(n); else log(`[james-marit-new-render] ignoring ${name}=${v} (invalid)`);
}
function applyInt(
  name: string,
  set: (n: number) => void,
...
applyFloat('--line-opacity', (n) => { LINE_OPACITY = n; }, (n) => Number.isFinite(n) && n > 0 && n <= 1);
{
  ...
  if (v !== null) {
    ...
    catch (e) { log(`[james-marit-new-render] ignoring --color-scheme=${v} (${(e as Error).message})`); }
  }
}
{
  const v = flagValue('--bg');
  if (v === 'white' || v === 'black') BG = v;
  else if (v !== null) log(`[james-marit-new-render] ignoring --bg=${v} (expected "white" or "black")`);
}
...
if (depthArg !== undefined) {
  ...
  if (Number.isFinite(n) && n >= 1) {
    DEPTH = n;
    log(`[james-marit-new-render] depth=${DEPTH} (from command line)`);
  } else {
    log(`[james-marit-new-render] ignoring non-integer depth arg "${depthArg}"`);
  }
}
if (DEPTH === VIEW_PRESET.previewDepth && depthArg === undefined && process.stdin.isTTY) {
  ...
  if (answer !== '') {
    ...
    else log(`[james-marit-new-render] invalid depth "${answer}", using default ${DEPTH}`);
  }
}

log(`[james-marit-new-render] depth=${DEPTH}, max dim=${MAX_DIM}, splat=${SPLAT_RADIUS}, ` +
...
const cachePath = `${CACHE_DIR}james-marit-new-depth${DEPTH}-${cacheKey}.acc`;
...
const outputFile = outputPath('james-marit-new', `james-marit-new${sTag}${aTag}-depth${DEPTH}-${imgW}x${imgH}${splatTag}${modeTag}.png`);
