/**
 * sl2c-render-limit-set.ts
 *
 * Offline density-image render of an SL(2,C) limit set on either the
 * stereographic plane or the Riemann sphere (orthographic top-down).
 *
 * Same two-phase shape as the sp6 renderer:
 *   1. accumulate — stream the non-backtracking word tree by DFS (memory
 *      O(depth)), project each visited state via the chosen embedding +
 *      autofit orthographic camera, integer-deposit into a 2D accumulator.
 *      Cache the grid to scripts/cache/<key>.acc for reuse.
 *   2. render — tone-map (log + percentile clip), write PNG.
 *
 *   node scripts/sl2c-render-limit-set.ts [depth] [flags...]
 *
 *   --example <id>          choose example from src/sl2c/examples.ts
 *   --embedding sphere|plane
 *   --max-dim N             long-side image dimension (default 8192)
 *   --color-scheme <spec>   'grayscale' | 'last-gen' | 'kth-last:k'
 *   --bg white|black
 *   --tone <p>              percentile clip in (0, 1]; default 0.999
 *   --gamma <g>             >0; reshape tone curve via t^(1/g)
 *   --refresh               ignore the .acc cache
 *
 * Re-runs with the same example/depth/maxDim/colorScheme/embedding hit the
 * cache and skip phase 1. Tone curve, gamma, and bg are render-only — they
 * don't invalidate the cache.
 *
 * Sphere embedding offline currently uses the same orthographic xy autofit
 * as the plane: it'll look like a top-down view of S². Good for fully-3D
 * Cantor sets; mediocre for fuchsian curves that live on a great circle.
 * Use the browser preview for free orbit-control framing on the sphere.
 */
...
import {
  ...
  EXAMPLES,
  type MobiusExample,
} from '../src/sl2c/examples.ts';
import { makeMobiusAction } from '../src/sl2c/action.ts';
import {
  sphereEmbedding,
  planeEmbedding,
} from '../src/sl2c/embedding.ts';
...
import type { ViewPreset, EmbeddingName } from '../src/sl2c/viewPreset.ts';

// ─── EDITABLE defaults ─────────────────────────────────────────────────────

const DEFAULT_EXAMPLE_ID = 'riley-2i';
const DEFAULT_EMBEDDING: 'sphere' | 'plane' = 'plane';
...
let   EMBEDDING_NAME: EmbeddingName = DEFAULT_EMBEDDING;
...
const VIEW_PRESET_PATH = fileURLToPath(new URL('./sl2c-view-preset.json', import.meta.url));
if (existsSync(VIEW_PRESET_PATH)) {
  try {
    VIEW_PRESET = JSON.parse(readFileSync(VIEW_PRESET_PATH, 'utf8'));
    process.stderr.write(`[sl2c-render] loaded sl2c-view-preset.json` +
      ` (exampleId=${VIEW_PRESET!.exampleId}, embedding=${VIEW_PRESET!.embedding},` +
      ` previewDepth=${VIEW_PRESET!.previewDepth})\n`);
    // Preset wins for choice of example + embedding (the camera was framed
    // against a specific orbit + view). CLI still overrides below.
    EXAMPLE_ID     = VIEW_PRESET!.exampleId;
    EMBEDDING_NAME = VIEW_PRESET!.embedding;
    if (typeof VIEW_PRESET!.colorScheme === 'string') {
      try {
        getScheme(VIEW_PRESET!.colorScheme);
        COLOR_SCHEME = VIEW_PRESET!.colorScheme;
      } catch (e) {
        process.stderr.write(`[sl2c-render] ignoring preset.colorScheme=${VIEW_PRESET!.colorScheme}: ${(e as Error).message}\n`);
      }
    }
  } catch (e) {
    process.stderr.write(`[sl2c-render] WARNING: ignoring malformed sl2c-view-preset.json: ${(e as Error).message}\n`);
    VIEW_PRESET = null;
  }
}
...
function applyFloat(
  ...
  if (predicate(n)) set(n); else log(`[sl2c-render] ignoring ${name}=${v} (invalid)`);
}
function applyInt(
  name: string,
  set: (n: number) => void,
...
applyFloat('--gamma',   (n) => { TONE_GAMMA = n; }, (n) => Number.isFinite(n) && n > 0);
{
  ...
  if (v !== null) {
    ...
    catch (e) { log(`[sl2c-render] ignoring --color-scheme=${v} (${(e as Error).message})`); }
  }
}
{
  const v = flagValue('--bg');
  if (v === 'white' || v === 'black') BG = v;
  else if (v !== null) log(`[sl2c-render] ignoring --bg=${v} (expected "white" or "black")`);
}
{
  const v = flagValue('--embedding');
  if (v === 'sphere' || v === 'plane') EMBEDDING_NAME = v;
  else if (v !== null) log(`[sl2c-render] ignoring --embedding=${v} (expected "sphere" or "plane")`);
}
{
  ...
  if (v !== null) {
    if (EXAMPLES.some((e) => e.id === v)) EXAMPLE_ID = v;
      ...
      log(`[sl2c-render] unknown example "${v}". available:`);
      for (const e of EXAMPLES) log(`     ${e.id.padEnd(28)} ${e.label}`);
      process.exit(1);
    }
  }
}
...
if (depthArg !== undefined) {
  ...
  if (Number.isFinite(n) && n >= 1) {
    DEPTH = n;
    log(`[sl2c-render] depth=${DEPTH} (from command line)`);
  } else {
    log(`[sl2c-render] ignoring non-integer depth arg "${depthArg}"`);
  }
}
if (DEPTH === DEFAULT_DEPTH && depthArg === undefined && process.stdin.isTTY) {
  ...
  if (answer !== '') {
    ...
    else log(`[sl2c-render] invalid depth "${answer}", using default ${DEFAULT_DEPTH}`);
  }
}

// ─── Active example ────────────────────────────────────────────────────────

const ACTIVE: MobiusExample = exampleById(EXAMPLE_ID);

const EMBEDDING: SceneEmbedding =
  EMBEDDING_NAME === 'sphere' ? sphereEmbedding : planeEmbedding;

log(`[sl2c-render] example=${ACTIVE.label} (${EXAMPLE_ID})  embedding=${EMBEDDING_NAME}`);
log(`[sl2c-render] depth=${DEPTH}, max dim=${MAX_DIM}, splat=${SPLAT_RADIUS}, ` +
...
function buildCacheKey(): string {
  // Note: bg, tone, gamma are render-only — NOT in the key. splatRadius IS.
...
const cachePath = `${CACHE_DIR}sl2c-${EXAMPLE_ID}-${EMBEDDING_NAME}-depth${DEPTH}-${cacheKey}.acc`;
...
function accumulate(action: GroupAction, basepoint: Float64Array): AccumulateResult {
  ...
  log(`Streaming DFS depth=${DEPTH} → ${imgW}×${imgH}×${K} accumulator ` +
      `(${formatCount(total)} total nodes, scheme=${scheme.name})...`);
...
if (!loadedFromCache) {
  ...
  const action = makeMobiusAction(ACTIVE.generators);
...
const outputFile =
  `sl2c-${EXAMPLE_ID}-${EMBEDDING_NAME}-depth${DEPTH}-${imgW}x${imgH}${splatTag}${outputSuffix}.png`;
