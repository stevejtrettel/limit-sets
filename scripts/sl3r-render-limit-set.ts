/**
 * sl3r-render-limit-set.ts
 *
 * Offline density-image render of an SL(3,R) / PGL(3,R) limit set on RP².
 *
 * Two phases, same shape as the sp6 / sl2c renderers:
 *   1. accumulate — stream the non-backtracking word tree by DFS (memory
 *      O(depth)); for reflection / Coxeter groups this emits exactly the
 *      reduced words. Project each visited state via the chosen embedding
 *      (sphere = identity on S², plane = affine chart z-denominator) +
 *      autofit orthographic camera; integer-deposit into a 2D accumulator.
 *      Cache the grid to scripts/cache/<key>.acc.
 *   2. render — tone-map (log + percentile clip), write PNG.
 *
 *   node scripts/sl3r-render-limit-set.ts [depth] [flags...]
 *
 *   --example <id>          choose example from src/sl3r/examples.ts
 *   --embedding sphere|plane (default plane)
 ...
 *
 * If scripts/sl3r-view-preset.json exists (written by the browser demo's
 * "save view" button), the script uses its perspective camera instead of
 * the orthographic auto-fit. Output filename gains a `-view` suffix.
 */
...
import type { ViewPreset, EmbeddingName } from '../src/sl3r/viewPreset.ts';

// ─── EDITABLE defaults ─────────────────────────────────────────────────────

const DEFAULT_EXAMPLE_ID = 'tri-334-d1.0';
...
const VIEW_PRESET_PATH = fileURLToPath(new URL('./sl3r-view-preset.json', import.meta.url));
if (existsSync(VIEW_PRESET_PATH)) {
  try {
    VIEW_PRESET = JSON.parse(readFileSync(VIEW_PRESET_PATH, 'utf8'));
    process.stderr.write(`[sl3r-render] loaded sl3r-view-preset.json` +
    ...
    if (typeof VIEW_PRESET!.colorScheme === 'string') {
      try {
        ...
        process.stderr.write(`[sl3r-render] ignoring preset.colorScheme=${VIEW_PRESET!.colorScheme}: ${(e as Error).message}\n`);
      }
    }
  } catch (e) {
    process.stderr.write(`[sl3r-render] WARNING: ignoring malformed sl3r-view-preset.json: ${(e as Error).message}\n`);
    VIEW_PRESET = null;
  }
}
...
function applyFloat(
  ...
  if (predicate(n)) set(n); else log(`[sl3r-render] ignoring ${name}=${v} (invalid)`);
}
function applyInt(
  name: string,
  set: (n: number) => void,
...
applyFloat('--gamma',   (n) => { TONE_GAMMA = n; }, (n) => Number.isFinite(n) && n > 0);
{
  ...
  if (v !== null) {
    ...
    catch (e) { log(`[sl3r-render] ignoring --color-scheme=${v} (${(e as Error).message})`); }
  }
}
{
  const v = flagValue('--bg');
  if (v === 'white' || v === 'black') BG = v;
  else if (v !== null) log(`[sl3r-render] ignoring --bg=${v} (expected "white" or "black")`);
}
{
  const v = flagValue('--embedding');
  if (v === 'sphere' || v === 'plane') EMBEDDING_NAME = v;
  else if (v !== null) log(`[sl3r-render] ignoring --embedding=${v} (expected "sphere" or "plane")`);
}
{
  ...
  if (v !== null) {
    if (EXAMPLES.some((e) => e.id === v)) EXAMPLE_ID = v;
      ...
      log(`[sl3r-render] unknown example "${v}". available:`);
...
if (depthArg !== undefined) {
  ...
  if (Number.isFinite(n) && n >= 1) {
    DEPTH = n;
    log(`[sl3r-render] depth=${DEPTH} (from command line)`);
  } else {
    log(`[sl3r-render] ignoring non-integer depth arg "${depthArg}"`);
  }
}
if (DEPTH === DEFAULT_DEPTH && depthArg === undefined && process.stdin.isTTY) {
  ...
  if (answer !== '') {
    ...
    else log(`[sl3r-render] invalid depth "${answer}", using default ${DEFAULT_DEPTH}`);
  }
}

// ─── Active example ────────────────────────────────────────────────────────

const ACTIVE: SL3RExample = exampleById(EXAMPLE_ID);

const EMBEDDING: SceneEmbedding =
  EMBEDDING_NAME === 'sphere' ? sphereEmbedding : planeEmbedding;

log(`[sl3r-render] example=${ACTIVE.label} (${EXAMPLE_ID})  embedding=${EMBEDDING_NAME}`);
log(`[sl3r-render] depth=${DEPTH}, max dim=${MAX_DIM}, splat=${SPLAT_RADIUS}, ` +
...
function buildCacheKey(): string {
  ...
  if (VIEW_PRESET) {
    keyObj.mode = 'preset';
    keyObj.preset = { camera: VIEW_PRESET.camera, viewport: VIEW_PRESET.viewport };
  } else {
    keyObj.mode = 'auto';
  }
...
const cachePath = `${CACHE_DIR}sl3r-${EXAMPLE_ID}-${EMBEDDING_NAME}-depth${DEPTH}-${cacheKey}.acc`;
...
if (!loadedFromCache) {
  ...
  const action = makeMat3Action(ACTIVE.generators, { involutions: ACTIVE.involutions });
...
const outputFile =
  `sl3r-${EXAMPLE_ID}-${EMBEDDING_NAME}-depth${DEPTH}-${imgW}x${imgH}${splatTag}${outputSuffix}.png`;
