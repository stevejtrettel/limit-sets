...
## Step 4f — branch containments (verifying GᵢK ⊆ K)

See `verify.md` for the math. Two parts, both grounded (the 11 containments +
boundedness are verified numerically: every `min H·(Gᵢr) ≥ 0`, every `(Gᵢr)₀ > 0`).

**Prereq:** add `E_U` to `copies.ts` (paper §1 matrix, quoted in `verify.md`).

**A. Visual — `branchCopies()` preset.** The 11 maps in `copies.ts`:
`G₀ = T⁻¹`; for `k=1..5`: `T⁻¹Sᵏ` and `T⁻¹SᵏE`
(`matmul6(T_INV_U, matpow(S_U,k))`, then `·E_U`). The `εk` signs are omitted —
projectively irrelevant for drawing, and our `bounded()` gate already handles
either sign. Add `'branch'` to `CopyMode` + the copies dropdown. The engine then
draws all 11 nested inside `K` with no other changes; the coloring view colors Λ
by which branch image contains each point. (Optionally include a faint base `K`
as the visible container.)

**B. Numeric — `verify.ts`.** A node-run script implementing `verify.md` checks
0–2 over `RAYS_U`/`FACETS_H`/`S_U`/`T_INV_U`/`E_U`, printing PASS/FAIL. This is
the paper's primal certificate (with the `εk` signs, so `H·(Gᵢr) ≥ 0` directly)
living in our code — it grounds the demo's correctness.
...
- [x] **Δ₀ dominance box** — the cube `[−1,1]³` containing ℙ(K) (verify.md
      check 1). A "Δ₀ dominance box" toggle in the Domains folder, gated to the
      u-basis e₀ chart.
- [x] **Step 4f-B — numeric verify** — `E_U` added to `copies.ts`; `verify.ts`
      runs checks 0–2 (`verify.md`) over the integer data and prints PASS/FAIL.
      `npm run verify-c32` → all pass. (Runs under plain Node — hence `copies.ts`
      now imports `./mat6.ts` with the explicit extension.)
- [ ] **Step 4f-A — visual branch copies** — a `branchCopies()` preset (the 11
      maps `T⁻¹Sᵏ(E)`) + a `'branch'` option, drawn nested inside `K`. Not yet
      built. Verified ready: all 11 `GᵢK ⊆ K` and bounded in e₀.
- [ ] **Step 4c** — (parked, NOT in the background) the S-symmetric "rosette"
      view. No grounded construction yet; revisit only with a real source.
