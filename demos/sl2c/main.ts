...
depth = currentExample.defaultDepth ?? DEFAULT_DEPTH;
...
panel.select({
  ...
  onChange: (id) => {
    loadExample(id);
    depth = currentExample.defaultDepth ?? DEFAULT_DEPTH;
    // Retune the slider's ceiling first: a wide alphabet blows up long before 14.
    slDepth.element.max = String(currentExample.maxDepth ?? MAX_DEPTH);
    slDepth.set(depth);
    regenerateOrbit(depth);
    rebuildMesh(true);
    updateUI();
...
const slDepth = panel.slider({
  label: 'depth N',
  min: 4, max: currentExample.maxDepth ?? MAX_DEPTH, step: 1, value: depth,
  onChange: (v) => {
    depth = v;
    regenerateOrbit(v);
    rebuildMesh(false);
