...
// ─── Degree-6 symplectic ─────────────────────────────────────────────────────
// Same core as orthogonal: derive the companion polynomials from α/β, check the
// (even-degree ⇒ palindromic) structure, and auto-seed. Every symplectic group
// has a limit set, so nothing is skipped.

const SYMPLECTIC_OPTS: HyperValidationOptions = {
  degree: 6, walk: 'free', reciprocal: 'palindrome',
};

export function validateSymplecticExample(ex: HyperExampleLike): ValidationResult {
  return validateOne(ex, SYMPLECTIC_OPTS);
}

export function validateAllSymplectic(examples: readonly HyperExampleLike[]): ValidationResult[] {
